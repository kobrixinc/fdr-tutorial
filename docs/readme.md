...
Ok, if you have the `fdr` object available, you are ready to roll. Here is how to create a [`Graph`](https://kobrixinc.github.io/fdr/api/interfaces/Graph.html) instance:
...
The `fdr.graph` function takes a "graph description" as a parameter. This is an object which at a minimum has to provide a backing store (a SPARQL endpoint) for the graph. Here, we are passing a global variable called `in_memory_store`, which is defined during the bootstrap of the tutorial application as an in memory triplestore, loaded with some data just for the tutorial's purpose (more on that below). The returned instance of [`Graph`](https://kobrixinc.github.io/fdr/api/interfaces/Graph.html) has the role of a smart local cache which knows how to manage changes and makes it easy to interact with a remote instance. 

Next, let's get and display some data. In RDF, the simplest unit of data is the `<subject, predicate, object>` triple. However, with FDR you don't work with triples directly, though you can always get to them if need be. Instead, the simplest piece of data is something called a [`Subject`](https://kobrixinc.github.io/fdr/api/interfaces/Subject.html) which aggregates all known triples that share the same subject. It is essentially a JavaScript object with properties. Another way to think of it is as a "Resource" from the Resource Description Framework.
...
In addition to `subjectId` and a `graph`, you can create several other important things from the `fdr` object because it aggregates several top-level factories as well as global configuration settings - see the [`FDR`](https://kobrixinc.github.io/fdr/api/interfaces/FDR.html) and [`RDFJS`] (https://kobrixinc.github.io/fdr/api/interfaces/RDFJS-1.html) interfaces for example. In particular it has something called a [`NameResolver`] (https://kobrixinc.github.io/fdr/api/interfaces/NameResolver.html) which lets you configure namespaces your application may be using. The `dbr:` in the snippet above is one such namespace configured by the tutorial sample app. 
...
This is what we have in `src/main.ts`, except that we do some FDR related initialization in between (see complete source code at [https://github.com/kobrixinc/fdr-tutorial/blob/master/src/main.ts](https://github.com/kobrixinc/fdr-tutorial/blob/master/src/main.ts)). 
...
2. Configure the global FDR environment, namely the default language for literal ("en") and some namespaces that we are going to use. The namespaces are configured as a set of prefixes that are then added to the ones available by default via a call to `fdr.resolver.prefixResolver.withPrefixes`. 
...
Using the in memory store, the initialization code creates a graph instance which is where data is ultimately read from and written to. Specifically, this will be an instance of [`LocalGraph`] ([ref link](https://kobrixinc.github.io/fdr/api/classes/LocalGraph.html), which is a full-blown cache and therefore in general not a lightweight object. Because of that, we tell VueJS **not** to make it reactive by calling the standard `Object.freeze` function. In a real application some care must be taken where such an object lives - you would want it in some scope whose lifetime is that of the whole application.
...
To use the FDR framework, one needs an instance of the [`Graph`](https://kobrixinc.github.io/fdr/api/interfaces/Graph.html) interface. And to create a `Graph` instance, one needs a backing store which supports querying for triples as well as adding and removing them. FDR provides out of the box implementations for standard SPARQL endpoints as a backing store, but to make it easier to run this tutorial we instead make use of an in memory triplestore as the backing database for the application. we use the Quadstore (https://www.npmjs.com/package/quadstore) module which provides the storage implementation and the Quadstore-Comunica (https://www.npmjs.com/package/quadstore-comunica) module which provides the SPARQL query engine.

The storage management details are behind the FDR [`Triplestore`](https://kobrixinc.github.io/fdr/api/interfaces/TripleStore.html) interface. The exact same interface is used for any triplestore endpoint and the tutorial's implementation of the in memory version can be used as an example for rolling your own implementations. We won't go into detail about this code (the [`src/inmemstore.ts`](https://github.com/kobrixinc/fdr-tutorial/blob/master/src/inmemstore.ts) implementation), but will gladly answer any questions you might have.
